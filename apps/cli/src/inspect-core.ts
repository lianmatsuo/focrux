 ...
 *
 * The record it reads is `<repo>/.focrux/`: working state of one machine.
 * What an *admitted ticket* adds — the key the record is filed under, the state
 * it is in, how it was admitted — reaches this module only through
 * {@link InspectSubject}, which reads from the ticket store where a name
 * resolves to one and answers from the attempts record itself otherwise.
 * Nothing here opens a ticket file.
 */
...
export async function runInspectCommand(input: InspectOptions): Promise<number> {
  ...
  if (args.verify !== null) {
    const subject = input.subject(storeDirectory, args.key);
  ...
  const report = buildInspectReport({
    storeDirectory,
    subject: input.subject(storeDirectory, args.key),
