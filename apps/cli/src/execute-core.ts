 ...
 * moves through, the key a ceiling hit is reported under — reaches this module
 * only through {@link TicketRuns}. Nothing in this file reads the ticket store
 * directly.
 */
...
/**
 * The ticket store, as `run` and `doctor` use it. A caller with no ticket
 * store to offer leaves this unset, and `--ticket` refuses with the reason.
 */
...
export interface ExecuteOptions {
  ...
  cwd: string;
  /**
   * The ticket store, where this build has one. Absent, `--ticket` is refused:
   * a run still takes a contract and a run configuration, which is the whole of
   * what the loop needs.
   */
  tickets?: TicketRuns | undefined;
...
export async function runExecuteCommand(options: ExecuteOptions): Promise<number> {
  ...
  if (args.ticket !== null && options.tickets === undefined) {
    throw new UsageError(
      "--ticket names an admitted ticket, which this build does not keep. Run the contract " +
        "itself: focrux run --contract <file> --config <file>, or focrux run --outcome \"...\"",
    );
  }
  ...
  if (local === null && args.ticket === null && args.contract === null) {
    throw new UsageError(
      (options.tickets === undefined ? "--contract or --outcome" : "--ticket, --contract or --outcome") +
        " is required (or --pr, to take the plan from a pull request)",
    );
  }
  ...
  if (args.relevel && (args.ticket === null || options.tickets === undefined)) {
    throw new UsageError(
  ...
  const admitted =
    args.ticket === null || options.tickets === undefined
      ? null
      : options.tickets.load({
          cwd: options.cwd,
          repo: args.repo,
          store: args.store,
          key: args.ticket,
  ...
  const configInput =
    admitted !== null
      ? options.tickets!.runConfig(admitted, override, args.publish)
      : local !== null
        ? mergeRunConfig(
            {
              dir: local.store,
  ...
  const runConfig = {
    ...
    relevel_context: admitted !== null && args.relevel ? await options.tickets!.relevelContext(admitted, base.base_ref) : [],
  };

  /** Null on the `--contract` path: no ticket, so no history to count runs in. */
  const runsStarted = admitted === null ? null : options.tickets!.starting(admitted, args.relevel);
  ...
  if (admitted) {
    const state = options.tickets!.finished(admitted, result, new Date(), args.relevel);
...
export interface DoctorOptions {
  ...
  /** The commands this build carries, in the order its help lists them. Reported as the COMMANDS block. */
  commands: readonly string[];
  /**
   * A ticket key per ticket id, for the ceiling hits below. The full build reads
   * it from the ticket store; where there is none the id is its own label, which
   * is what the attempts record is keyed by anyway.
   */
...
export async function runDoctorCommand(options: DoctorOptions): Promise<number> {
  ...
  const keyFor = options.keyFor ? options.keyFor(store) : new Map<string, string>();
  ...
  if (options.args.json || !options.streams.isTTY) {
    ...
    lines.push("", ...renderCommands(options.commands));
    lines.push("");
    if (written) {
 ...
 * was the largest part of the JSON a person had to write by hand.
 *
 * Here rather than in `execute.ts` because none of it reads a ticket: it reads
 * the store's own `config.json`, which is working state of one machine, and the
 * run's identity is passed in. The full build passes an admitted ticket's; a
 * run with nothing admitted behind it passes the identity of its contract's
 * source (SCP-180), so both are judged by the same repository agreement.
 */
