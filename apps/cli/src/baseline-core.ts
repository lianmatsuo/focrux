...
import { formatDuration } from "./execute-core.js";
 ...
 *
 * A stopwatch over one person's own workflow on one machine. The one thing it
 * wants from the ticket store — whether anything is admitted here — reaches it
 * through {@link BaselineOptions.admitted}, which a caller with no ticket store
 * to ask leaves unset: nothing it could admit means every baseline it captures
 * is a first one.
 */
...
export interface BaselineOptions {
  ...
  now?: () => Date;
  /**
   * Whether this store already holds admitted work, which is what makes a
   * capture late (D-038). Supplied by the full build from its ticket store;
   * absent — in a build that admits nothing — no baseline is ever late.
   */
  admitted?: (storeDirectory: string) => boolean;
}
    ...
    switch (args.command) {
      case "start":
        after = startBaseline(before, {
          ...
          ref: args.ref,
          now,
          ticketsExist: input.admitted !== undefined && input.admitted(store),
        });
        break;
      case "pause":
