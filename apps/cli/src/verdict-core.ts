...
import { buildInspectReport, type InspectSubject, type ResolveSubject } from "./inspect-core.js";
 ...
 * The one thing an admitted ticket adds — how a reference a person typed
 * becomes the work it names — reaches this module as a {@link ResolveSubject},
 * which `verdict.js` reads from the ticket store where one exists and answers
 * from the attempts record otherwise. Nothing here opens a ticket file.
 */
...
export function knownFindings(dir: string, subject: InspectSubject): KnownFinding[] {
  ...
  const report = buildInspectReport({ storeDirectory: dir, subject, attempt: null });
  ...
  throw new UsageError(
    findings.length === 0
      ? `no findings are recorded for this review, so '${key}' cannot be one: ` +
          // Named rather than `sync`, which only one of the two builds has:
          // `inspect` is in both, and it is what shows whether this store holds
          // the bundles the findings would have been read from.
 ...
 *
 * `resolve` is the build's own record — the ticket store in one build, the
 * attempts in the other — and it is asked first, so a review an attempt filed
 * resolves exactly as it always did. A review nothing ran for reaches that
...
export async function runVerdictCommand(input: VerdictOptions): Promise<number> {
  ...
  const subject = subjectFor(dir, args.reference, input.resolve);
