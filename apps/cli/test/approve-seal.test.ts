 ...
 * object and `focrux edit` rewrites both, so a difference between them is a
 ...
 * right; `focrux run --ticket` checks it again, because that is the moment it
...
const scratch = mkdtempSync(join(tmpdir(), "focrux-approve-seal-"));
 ...
 *
 * Everything here beyond removing the key is what its `focrux edit --path` did:
...
describe("a contract is refused when it does not match the counter-seal beside it", () => {
  it("refuses a hand-edited draft, starts no attempt and leaves the ticket unapproved", async () => {
    ...
    handEditDraft(dir, "FCX-1", (contract) => {
      contract.scope.paths_allowed = ["**"];
    });
    const before = storeContents(dir);

    const streams = capture();
    expect(() => runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams, cwd: repo })).toThrow(
      UsageError,
    );
    ...
    await expect(
      runExecuteCommand({
        args: parseExecuteArgs(["--ticket", "FCX-1", "--repo", repo]),
        streams: capture(),
        cwd: repo,
      }),
  ...
  it("names every field that differs and sends the person to focrux edit", () => {
    ...
    let message = "";
    try {
      runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams: capture(), cwd: repo });
    ...
    expect(message).toContain("focrux edit");
    // The refusal is about these two files, and says which ticket's.
    expect(message).toContain("FCX-1.draft.json");
    expect(message).toContain("FCX-1.contract.json");
  ...
  it("refuses a hand-edited contract too: the check is not about which file was touched", () => {
    ...
    const path = join(dir, "tickets", "FCX-1.contract.json");
    const contract = JSON.parse(readFileSync(path, "utf8")) as StoredContract;
    contract.scope.paths_allowed = ["**"];
    writeFileSync(path, `${JSON.stringify(contract, null, 2)}\n`);
  ...
  it("reports no edits for a difference it refused, and the edits focrux edit made for one it took", async () => {
    ...
    expect(() =>
      runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams: refused, cwd: repo }),
    ).toThrow(UsageError);
    ...
    expect(readTicket(dir, "FCX-1").admission.edit_count).toBeNull();

    // The way out is the one the refusal names. `focrux edit` reports what it
    // applied — one glob for another — and rewrites both files, so they agree.
    const edited = capture();
    await runEditCommand({
      argv: ["FCX-1", "--repo", repo, "--path", "packages/search/api/**"],
      streams: edited,
      cwd: repo,
    });
    ...
    expect(readTicket(dir, "FCX-1").admission.edit_count).toBe(2);
    ...
    expect(runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams: approve, cwd: repo })).toBe(0);
  ...
  it("approves a contract focrux edit changed, sealing that scope and the level it re-derived", async () => {
    const { repo, dir } = admitted("seal-edited-approves");
    expect(readContract(dir, "FCX-1").level).toBe("P1");

    await runEditCommand({
      argv: ["FCX-1", "--repo", repo, "--path", "packages/auth/**"],
      streams: capture(),
      cwd: repo,
    });
    ...
    const sealed = readContract(dir, "FCX-1");
    ...
    expect(readDraftSnapshot(dir, "FCX-1")?.contract).toEqual(sealed);

    // And it is immutable from here: the way to change it is new work.
    await expect(
      runEditCommand({
        argv: ["FCX-1", "--repo", repo, "--path", "packages/billing/**"],
        streams: capture(),
        cwd: repo,
      }),
  ...
  it("refuses a ticket whose draft file was deleted, so the check cannot be opted out of", async () => {
    ...
    handEditContract(dir, "FCX-1", (contract) => {
      contract.scope.paths_allowed = ["**"];
    });
    rmSync(draftPath(dir, "FCX-1"));

    const approve = capture();
    let message = "";
    try {
      runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams: approve, cwd: repo });
      expect.unreachable("approve accepted a contract with no counter-seal");
    } catch (error) {
      expect(error).toBeInstanceOf(UsageError);
      message = (error as Error).message;
    }
    expect(message).toContain("FCX-1.draft.json is missing");
    expect(message).toContain("focrux edit FCX-1");
  ...
  it("says what it cannot read, rather than the parse error, and focrux edit puts it back", async () => {
    ...
    writeFileSync(draftPath(dir, "FCX-1"), '{"contract": {,,,\n');
    ...
    expect(message).toContain("FCX-1.draft.json cannot be read");
    ...
    const edited = capture();
    await runEditCommand({
      argv: ["FCX-1", "--repo", repo, "--path", "packages/search/**"],
      streams: edited,
      cwd: repo,
    });
    expect(edited.err.join("")).toContain("FCX-1.draft.json cannot be read");
    expect(runApproveCommand({ argv: ["FCX-1", "--repo", repo], streams: capture(), cwd: repo })).toBe(0);
    expect(readContract(dir, "FCX-1").scope.paths_allowed).toEqual(["packages/search/**"]);
    expect(readDraftSnapshot(dir, "FCX-1")?.contract).toEqual(readContract(dir, "FCX-1"));
  });
  ...
  it("refuses to bind an attempt to a contract hand-edited after it was approved", async () => {
    ...
    expect(message).toContain("FCX-1 cannot be run");
  ...
  it("leaves a store from before counter-seals alone: not approved, not run, not accused", async () => {
    ...
    asEditedByThePreviousVersion(dir, "FCX-1");
    expect(readTicket(dir, "FCX-1").admission.counter_sealed_at).toBeNull();
    ...
    const loaded = loadAdmitted(repo, repo, null, "FCX-1");
  ...
  it("counter-seals a ticket from before them at its first edit, keeping the earlier count", async () => {
    ...
    await runEditCommand({
      argv: ["FCX-1", "--repo", repo, "--outcome", "Search results are paginated at 25 per page."],
      streams: capture(),
      cwd: repo,
    });
