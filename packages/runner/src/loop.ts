...
import { PinnedCheckSchema, runPinnedChecks, type PinnedCheck } from "./checks.js";
import {
...
async function runLockedTicket(
  ...
  if (config.relevel) {
    ...
    const inWorktree = { cwd: workspace.path, env: gitEnv(), timeoutMs: 120_000 };
    const pushed = await run(["git", "rev-parse", "--verify", "--quiet", `${remote}^{commit}`], {
      cwd: config.repository_root,
      env: gitEnv(),
      timeoutMs: 120_000,
    });
    if (pushed.code === 0 && pushed.stdout.trim().length > 0) {
      const tip = pushed.stdout.trim();
      const local = (await run(["git", "rev-parse", "HEAD"], inWorktree)).stdout.trim();
      if (local !== tip) {
        const ahead = (await run(["git", "merge-base", "--is-ancestor", tip, "HEAD"], inWorktree)).code === 0;
        ...
        if (ahead && foreign.length === 0) {
          await run(["git", "reset", "--hard", tip], inWorktree);
          progress(
  ...
  const judgeRelevel = async (input: {
    ...
    const moved = await run(["git", "diff", "--name-only", input.base_before, baseCommit], {
      cwd: config.repository_root,
      env: gitEnv(),
      timeoutMs: 120_000,
    });
