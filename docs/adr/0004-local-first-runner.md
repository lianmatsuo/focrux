...
The compensating controls are a command allow-list, a write guard that decides a tool call before it runs, prohibited-path detection at the seal and over the transcript, a scrubbed environment, and egress logging. There is no filesystem jail: the worktree root is a recorded field, not a boundary a process cannot cross. [docs/08](../08-security-autonomy-and-data.md) states exactly what is prevented and what is detected.
