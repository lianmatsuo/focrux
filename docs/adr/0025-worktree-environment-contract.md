...
1. **Materialization manifest.** The repository declares the files and directories every worktree needs that git does not track: the `.env` family, local certificates, seed data, tool caches. `focrux doctor` proposes the manifest by inspecting the person's checkout, and the person confirms it.
...
- Materialization copies files; it does not start services. `doctor` refuses a repository whose verification command starts a service (`verification_requires_service`) and reports one that merely depends on containers (`undeclared_service_dependency`). A repository whose tests need a running database is outside the support boundary ([D-013](../11-open-decisions.md)).
