...
The **CLI tarball** is what a design partner installs. `tooling/package/pack.mjs` builds the workspace, bundles the CLI to one file, stages the write-guard hook, a version manifest and a licence notice beside it, and archives the result with a published SHA-256. `.github/workflows/release.yml`, on a version tag or a manual dispatch naming one, re-gates the exact commit (build, typecheck, test, lint), checks the tag against the CLI's own package manifest, packs the tarball, verifies the write-guard hook is inside it, attests build provenance once the repository is public, and drafts a GitHub release carrying the tarball and its digest. Nothing auto-updates: a new version is a new tarball and a message ([D-046](11-open-decisions.md)).

## Pull requests and merges

`main` is protected: every change arrives by pull request, admins included, and force-push and deletion are refused. An unsigned commit is accepted ([D-091](11-open-decisions.md)). One workflow, `.github/workflows/build.yml`, runs on every pull request and on a push to `main`: the turbo build, typecheck, test and lint above; the desktop runtime and licence check; a check that the corpus is well formed with current fixture diffs; the backlog, docs and diagram validators and the repository-validator unit tests that prove each can fail; a protected-paths check that a pull request touching a file that judges a change — the reviewer's blocking, remediation and decision-order tests, the runner's security test, the sample fixtures the reviewer is scored on — names the file and the rule it matched; and, on a pull request that touches `packages/review/`, the regression suite read against the recorded score. Its data lives under `.github/`: `corpus-pin.json` pins the public corpus commit the suite runs against, `protected-paths.json` lists the files a pull request may not touch, `regression-score.json` is the recorded score, and `scripts/` holds the checks themselves. None of this is a required check, so the gate above, run locally, is what a change passes before it merges. `.github/workflows/release.yml` runs separately, on a version tag or a manual dispatch naming one — see Packaging and release above.
