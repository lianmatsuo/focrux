...
A commit merges with the `Signed-off-by` of the person who submits it ([`CONTRIBUTING.md`](CONTRIBUTING.md)); a commit an agent wrote carries `Assisted-by: LLM` and no `Co-Authored-By` naming a model ([D-NEW-commit-convention](docs/11-open-decisions.md)). `.claude/settings.json` sets that trailer for Claude Code.
