...
The layout is in [docs/07](docs/07-monorepo-and-deployment.md), and each package's `README.md` says what it owns. `packages/review`, `packages/evaluation` and `packages/runner` each carry an `AGENTS.md` with the rules that bind work inside them: read it before changing anything there, because the rules below are only the ones that hold everywhere.
...
Every commit carries the `Signed-off-by` of the person who submits it ([`CONTRIBUTING.md`](CONTRIBUTING.md)); a commit an agent wrote carries `Assisted-by: LLM` and no `Co-Authored-By` naming a model ([D-NEW-commit-convention](docs/11-open-decisions.md)). `.claude/settings.json` sets that trailer for Claude Code.
...
- Issue tracker: `backlog/issues.json` is canonical; `scripts/sync_github_issues.py` can project it onto GitHub issues, and until it has, there are none. Do not create a second tracker or invent triage labels.
