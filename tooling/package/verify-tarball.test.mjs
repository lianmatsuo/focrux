...
function tarball(names) {
  const work = mkdtempSync(join(tmpdir(), "focrux-verify-"));
  scratch.push(work);
  const name = "focrux-0.0.0";
...
test("an archive carrying the hook passes, and its contents are listed", () => {
  const run = verify(tarball(["focrux.mjs", "guard-hook.js"]));
  assert.equal(run.status, 0, `${run.stdout}${run.stderr}`);
  assert.match(run.stdout, /focrux-0\.0\.0\/bin\/guard-hook\.js/);
  assert.match(run.stdout, /focrux-0\.0\.0\/bin\/focrux\.mjs/);
  assert.doesNotMatch(run.stdout, /::error::/);
});

test("an archive without the hook fails, naming what is missing", () => {
  const run = verify(tarball(["focrux.mjs"]));
...
test("a tarball that cannot be read fails rather than passing silently", () => {
  const run = verify(join(tmpdir(), "focrux-no-such-archive.tgz"));
